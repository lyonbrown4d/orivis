...
func main() {
	...
	cmd.Flags().Duration("transport-request-timeout", 0, "agent HTTP request timeout")
	cmd.Flags().Int("transport-max-idle-conns", 0, "agent HTTP maximum idle connections")
	cmd.Flags().Int("transport-max-idle-conns-per-host", 0, "agent HTTP maximum idle connections per host")
	cmd.Flags().Duration("transport-idle-conn-timeout", 0, "agent HTTP idle connection timeout")
	cmd.Flags().Duration("transport-tls-handshake-timeout", 0, "agent HTTP TLS handshake timeout")
	cmd.Flags().Duration("transport-response-header-timeout", 0, "agent HTTP response header timeout")
	cmd.Flags().Int("transport-retry-attempts", 0, "agent HTTP retry attempts")
	cmd.Flags().Duration("transport-retry-base-delay", 0, "agent HTTP retry base delay")
	cmd.Flags().Duration("transport-retry-max-delay", 0, "agent HTTP retry max delay")
	cmd.Flags().Float64("transport-retry-jitter-ratio", 0, "agent HTTP retry jitter ratio")
	cmd.Flags().Bool("transport-gzip-results", false, "gzip agent result report requests")
