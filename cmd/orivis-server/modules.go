...
const serverRunStopTimeout = 10 * time.Second

func buildServerVersion() string {
	return buildinfo.Version
}

func defaultServerRunStopTimeout() time.Duration {
	return serverRunStopTimeout
}
...
func newServerModules(
	...
	configModule := newServerConfigModule(cmd, configFile)
	lifecycleContextModule := newServerLifecycleContextModule(ctx)
	loggingModule := newServerLoggingModule(configModule)
	storeModule := newServerStoreModule(configModule, loggingModule, lifecycleContextModule)
	...
	notificationModule := newServerNotificationModule(configModule, loggingModule, eventModule, cacheModule, storeModule, observabilityModule)
	...
	return []dix.Module{httpModule, mdnsModule, retentionModule, notificationModule, concurrencyModule}
}

func newServerConfigModule(cmd *cobra.Command, configFile string) dix.Module {
...
func newServerLifecycleContextModule(ctx context.Context) dix.Module {
	return dix.NewModule("lifecycle-context",
		dix.WithModuleProviders(
			dix.Value(ctx),
		),
	)
...
func newServerLoggingModule(configModule dix.Module) dix.Module {
	return dix.NewModule("logging",
...
func newServerStoreModule(configModule, loggingModule, lifecycleContextModule dix.Module) dix.Module {
	return dix.NewModule("store",
		dix.WithModuleImports(configModule, loggingModule, lifecycleContextModule),
		dix.WithModuleProviders(
			dix.ProviderErr3(store.OpenDBWithContext),
			dix.Provider1(store.NewRepositories),
			dix.Provider1(store.NewIDGenerator),
			dix.ProviderErr4(store.NewWithContext),
		),
		dix.WithModuleHooks(
...
func newServerNotificationModule(
	configModule,
	loggingModule,
	eventModule,
	cacheModule,
	...
	return dix.NewModule("notification",
		dix.WithModuleImports(configModule, loggingModule, eventModule, cacheModule, storeModule, observabilityModule),
		dix.WithModuleProviders(
			dix.ProviderErr6(notification.NewManager),
		),
		dix.WithModuleHooks(
