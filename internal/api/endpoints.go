...
func NewAgentEndpoint(cfg config.Config, storage *store.Store) httpx.Endpoint {
	return &agentEndpoint{cfg: cfg, store: storage}
}

func NewDefaultEndpoints(cfg config.Config, storage *store.Store) *collectionlist.List[httpx.Endpoint] {
	return collectionlist.NewList(
		NewDashboardEndpoint(cfg, storage),
		NewMetadataEndpoint(cfg, storage),
		NewHealthEndpoint(),
		NewAgentEndpoint(cfg, storage),
	)
}
